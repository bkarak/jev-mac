...
let help = """
jev-mac — typed-decision engine on Apple foundation models
...
USAGE
  jev-mac check [--measure]                      describe the Apple foundation models (--measure times them)
  ...
  jev-mac bench --suite fizzbuzz                          300 typed decisions with exact labels
...
struct Main {
    static func main() async {
        ...
        do {
            switch command {
            case "check":
                try await Commands.check(Args(rest, switches: ["measure"], options: []))
            case "presets":
            ...
            case "help", "--help", "-h":
                print(help)
            default:
                throw UsageError("unknown command '\(command)'. Run: jev-mac help")
            }
        } catch {
            printErr("jev-mac: " + Agent.explain(error))
