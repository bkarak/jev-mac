...
enum Live {
    static let enabled = ProcessInfo.processInfo.environment["JEV_LIVE"] == "1"

    /// The engine exactly as `jev predict` configures it by default.
...
actor LiveLedger {
    ...
    func report() -> (summary: String, misses: String) {
        ...
        var out = ["", "── jev live validity report " + String(repeating: "─", count: 44)]
...
struct LiveReport: SuiteTrait, TestScoping {
    ...
    func provideScope(for test: Test, testCase: Test.Case?, performing function: @Sendable () async throws -> Void) async throws {
        ...
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("jev-live-report.txt")
