...
/// Runs the real `jev` binary. None of these reach the model: every case
/// either needs no model or fails validation first.
@Suite("CLI", .enabled(if: CLI.binary != nil, "needs the jev binary next to the test bundle"))
struct CLITests {
    struct Case: Sendable, CustomTestStringConvertible {
        ...
        var testDescription: String { (["jev"] + args).joined(separator: " ") + (stdin.isEmpty ? "" : " < stdin") }
    }
    ...
    func run(_ c: Case) throws {
        ...
        if c.status == 2 { #expect(r.stderr.hasPrefix("jev: "), "errors are prefixed for scripts") }
    }
    ...
    @Test func checkReportsEveryModel() throws {
        let r = try CLI.run(["check"])
        #expect(r.status == 0)
        for line in ["on-device (general)", "on-device (tagging)", "private cloud compute"] { #expect(r.stdout.contains(line)) }
    }
