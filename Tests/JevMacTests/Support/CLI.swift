...
/// Runs the `jev-mac` binary built next to this test bundle.
enum CLI {
    static let binary: URL? = {
        if let path = ProcessInfo.processInfo.environment["JEV_MAC_BINARY"] { return URL(fileURLWithPath: path) }
        let candidate = Bundle(for: BundleToken.self).bundleURL
            .deletingLastPathComponent().appendingPathComponent("jev-mac")
    ...
    /// Runs `jev-mac` with `args`, feeding `stdin` (closed afterwards so the
    ...
    enum CLIError: Error, CustomStringConvertible {
        case missingBinary
        var description: String { "jev-mac binary not found next to the test bundle; set JEV_MAC_BINARY" }
    }
}
...
func tempFile(_ contents: String, ext: String = "json") throws -> String {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("jev-mac-test-\(UUID().uuidString).\(ext)")
